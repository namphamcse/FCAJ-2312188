...
Tạo internet-facing ALB `shopsflow-alb` trên hai public subnet. Tạo HTTP `80` listener (và HTTPS `443` khi đã cấu hình certificate), IP target group trên port `8080` cho ECS Fargate và cấu hình health check endpoint của backend. Gắn ECS service vào target group để task được đăng ký tự động.
